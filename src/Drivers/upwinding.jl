function tsw_testcase_entropy(ranks,nprocs,options,n,dt,tF,p,g,H0,f,L,domain,u,h,B,convert2seconds,
  ...
  const_jac::Bool,upwinding::Bool,nls_tols)

    i_am_main(ranks) && println("using upwinding -- entropy will not be conserved")
    ...
    res_prog((t,dt),(u0,h0,B0),(u,h,B),(T),(b1),(b2),(b3),(F,Φ,q,ω),(v,w,r), (b)) = (
          ...
          + ∫( dt*0.5*(my_sign_jac∘( (F⋅n_Λ).plus )*( (F⋅n_Λ).plus ) )*jump(b1*n_Λ )⋅jump(r*n_Λ )   )dΛ # B upwinding
          - ∫( dt*0.5*(my_sign_jac∘( (F⋅n_Λ).plus )*( (v⋅n_Λ).plus ) )*jump(b1*n_Λ )⋅jump(T*n_Λ )   )dΛ # u upwinding
    )


    c = 0.5
    jac_prog((t,dt),(u0,h0,B0),(u,h,B),(du,dh,dB),(v,w,r),b,(F,Φ,q,ω),b3) = ( ## Not a function of diagnostic variables
            ∫( du⋅v  )dΩ
          ...
          + ∫( (c*dt)*0.5*(my_sign_jac∘( (F⋅n_Λ).plus )*( ( (h0*du)⋅n_Λ).plus)  )*(jump(b*n_Λ )⋅jump(r*n_Λ ) )   )dΛ # B upwinding
          - ∫( (c*dt)*0.5*(my_sign_jac∘( (F⋅n_Λ).plus )*( (v⋅n_Λ).plus ) )*(jump(b*n_Λ )⋅jump( (0.5*dh)*n_Λ ))   )dΛ # u upwinding
    )
    ...
    # + ∫( dB*r )dΩ
    # + ∫( ((c*dt)*b*h0)*(∇⋅du)*r )dΩ
    # + ∫( (c*dt)*0.5*(my_sign∘( (F⋅n_Λ).plus )*( ( (h0*du)⋅n_Λ).plus)  )*(jump(b*n_Λ )⋅jump(r*n_Λ ) )   )dΛ # B upwinding
    # - ∫( (c*dt)*0.5*(my_sign∘( (F⋅n_Λ).plus )*( (v⋅n_Λ).plus ) )*(jump(b*n_Λ )⋅jump( (0.5*dh)*n_Λ ))   )dΛ # u upwinding
      # )
    ...
    entropy_upwind(b,F) = sum( ∫( (my_sign_jac∘( (F⋅n_Λ).plus )*( (F⋅n_Λ).plus ) )*jump(b*n_Λ )⋅jump(b*n_Λ )   )dΛ )
    corio(f) =  sum( ∫( f )dΩ)
    ...
    createpvd(ranks,out_dir*"/tsw_$simName") do pvd
      ...
      while !isnothing(it)
        ...
        i_am_main(ranks) && println("S upwind = ", Supwind)
        i_am_main(ranks) && println("dSdt = ", Sinternal+Scenter+Supwind)
...
function restarted_tsw_testcase_entropy(ranks,nprocs,options,out_dir,out_freq,tF,
  const_jac::Bool,upwinding::Bool,nls_tols,CFL,n)


    i_am_main(ranks) && println("using upwinding -- RESTARTED -- entropy will not be conserved")

    df = wload(datadir(out_dir, "dofs.jld2"))
    df2 = wload(datadir(out_dir, "tsw_entropy.jld2"))

    @unpack t, p, g, H0, f, L, simName, domain, counter, convert2seconds = df
    ...
    C = CFL/(uu*p^2)
    dt = C*dx
    i_am_main(ranks) && println("n = ", n, " dt = ", dt)
    ...
    i_am_main(ranks) && println("made spaces")
