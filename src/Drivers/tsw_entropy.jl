...
#       const_jac=false,
#       out_loc="tsw_entropy_tmp",out_freq=1,
...
#   main_tsw_entropy(dicts,ranks,nprocs,options,out_loc,testcase,upwinding,conserving)

#   GridapPETSc.Finalize()
#   GridapPETSc.gridap_petsc_gc()
# end

# function main_tsw_entropy(dicts,ranks,nprocs,options,out_loc,testcase,upwinding,conserving)
...
#     s = run_tsw_entropy(d,ranks,nprocs,options,out_dir,testcase,upwinding,conserving)
...
# function run_tsw_entropy(case::Dict,ranks,nprocs,options,out_dir,testcase::tswtestcase,upwinding,conserving)
using DrWatson
using PoissonIntegrator2
using Gridap
using GridapSolvers
...
tF = 1.0
CFL = 0.2
out_loc = "tsw_entropy_tmp"
out_freq = 1
nls_tols = (;atol=1e-16,rtol=1e-11,maxiter=50)
const_jac = false
testcase = instability
  ...
  # results = run_tsw_entropy(ranks,nprocs,options,testcase,n,dt,tFd,p,g,H0,f,u,h,B,L,
  #                           domain,convert2seconds,out_dir,out_freq,simName,const_jac,upwinding,conserving,nls_tols)
...
#                   const_jac::Bool,upwinding,conserving::Bool,nls_tols)

#   i_am_main(ranks) && println("running tsw $(simName) testcase -- entropy conservation")

#   results = tsw_testcase_entropy(ranks,nprocs,options,n,dt,tFd,p,g,H0,f,L,
#               domain,u,h,B,convert2seconds,out_dir,simName,out_freq,const_jac,upwinding,nls_tols)
...
# function run_tsw_entropy(case::Dict,ranks,nprocs,options,out_dir,testcase::tswrestart,upwinding,consvering::Bool)
...
#                     tF,const_jac::Bool,upwinding,nls_tols,CFL,n)
...
#   const_jac::Bool,upwinding::Nothing,nls_tols)

#   i_am_main(ranks) && println("no upwinding")
...
#   dir_meta = datadir(out_dir*"/meta")
  ...
  createpvd(ranks,out_dir*"/tsw_$simName") do pvd
    ...
    while !isnothing(it)
      ...
      println("t = ", td, "; tF = ", tF)
      println("Normalised energy = ", (E-E0)/E0)
      println("Normalised entropy = ", (S-S0)/S0)
      println("dSdt = ", Sinternal+Scenter)
...
#                     const_jac::Bool,upwinding::Nothing,nls_tols,CFL,n)

#   i_am_main(ranks) && println("no upwinding -- RESTARTED")
