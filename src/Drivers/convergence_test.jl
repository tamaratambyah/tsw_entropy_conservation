...
function run_tsw_entropy(case::Dict,ranks,nprocs,options,out_dir,testcase::tswconvergence,upwinding_parms)
  ...
  results = convergence_test(ranks,nprocs,options,n,dt,tFd,p,g,H0,f,L,domain,
                            u,h,B,convert2seconds,out_dir,out_freq,const_jac,upwinding_parms,nls_tols)
...
function convergence_test(ranks,nprocs,options,n,dt,tF,p,g,H0,f,L,domain,u,h,B,convert2seconds,
  ...
  counter = 1
    ...
    while !isnothing(it)
      ...
      if mod(counter,out_freq) == 0
        ...
        output = @strdict el2_u el2_h el2_B T el2_uold el2_hold el2_Bold Told ts Cs
  ...
  results = @strdict el2_u el2_h el2_B T el2_uold el2_hold el2_Bold Told ts Cs
